...
# 部署 cert-manager
deploy-cert-manager: _helm-repos
...
deploy-postgres-operator: _helm-repos
...
deploy-provider:
    @echo "🏢 部署 Provider..."
    @kubectl create namespace provider 2>/dev/null || true
    @kubectl create secret generic keystore-password --from-literal=password=changeit -n provider 2>/dev/null || true
...
deploy-consumer:
    @echo "👥 部署 Consumer..."
    @kubectl create namespace consumer 2>/dev/null || true
...
up: k3s-start k3s-wait deploy-infra deploy-apps smoke-test
    ...
    @echo "  just logs-ns <ns> <pod>  # 查看 Pod 日志"
...
logs-keycloak:
    @kubectl logs -n provider -l app.kubernetes.io/name=keycloak --tail=50

# 查看 Provider Verifier 日志
logs-verifier:
    @kubectl logs -n provider -l app.kubernetes.io/name=verifier --tail=50

# 查看 MongoDB 日志
logs-mongodb:
    @kubectl logs -n provider mongodb-0 --tail=50

# 查看 PostgreSQL 日志
logs-postgres:
    @kubectl logs -n provider postgres-0 --tail=50
...
smoke-test:
    ...
    @echo ""
    @echo "  [Trust Anchor]"
    @kubectl get pods -n trust-anchor --no-headers 2>/dev/null | awk '{print "    " $$1 ": " $$3}'
    @echo ""
    @echo "  [Provider]"
    @kubectl get pods -n provider --no-headers 2>/dev/null | awk '{print "    " $$1 ": " $$3}'
    @echo ""
    @echo "  [Consumer]"
    @kubectl get pods -n consumer --no-headers 2>/dev/null | awk '{print "    " $$1 ": " $$3}'
    @echo ""
    @kubectl get pods -A --no-headers 2>/dev/null | awk '{print $$3}' | grep -cvE "Running|Completed" | awk '{if ($$1 > 0) print "  ⚠️  " $$1 " 个 Pod 异常"; else print "  ✅ 所有 Pod 正常"}'
...
uninstall-all:
    @echo "🧹 卸载所有组件..."
    -helm uninstall consumer -n consumer 2>/dev/null
    -helm uninstall provider -n provider 2>/dev/null
    -helm uninstall trust-anchor -n trust-anchor 2>/dev/null
    -helm uninstall postgres-operator -n postgres-operator 2>/dev/null
...
clean-namespaces:
    @echo "🧹 删除命名空间..."
    -kubectl delete namespace consumer provider trust-anchor postgres-operator cert-manager 2>/dev/null
    @echo "✅ 命名空间删除完成"
